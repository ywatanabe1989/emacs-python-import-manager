...
(defun pim-enable-auto-mode
  ...
  (when
    ...
    (add-hook 'before-save-hook #'pim-fix-imports nil t)))
                                        ; Changed from #'pim to #'pim-fix-imports
...
(defcustom pim-isort-path
  (or
   (bound-and-true-p python-isort-command)
...
(add-to-list 'load-path
             (concat pim--script-dir "predefined-packages"))
...
(defun pim--find-flake8
  ...
  (or pim-flake8-path
      (executable-find "flake8")
      (user-error
       "Cannot find flake8. Please install it or set pim-flake8-path")))
...
(defun pim--get-flake8-output
    (temp-file &optional args)
...
(defun pim--find-unused-modules
  ...
  (let*
      ((temp-file
        (pim--copy-contents-as-temp-file))
       (output
        (pim--get-flake8-output temp-file
                                '("--select=F401")))
       modules)
    (with-temp-buffer
      (insert output)
      (goto-char
       (point-min))
      (while
          (re-search-forward "F401 '\\([^']+\\)' imported but unused"
                             nil t)
        (let
            ((module
              (match-string 1)))
  ...
  (save-excursion
    (let
      ...
      (goto-char
       (point-min))
      (while
          (re-search-forward "^\\(from .* import .*\\|import .*\\)$"
                             nil t)
        (let
            ((line
              (match-string 0)))
          (if
              (gethash line imports)
              (kill-whole-line)
...
(defun pim--find-undefined
  ...
  (let*
    ...
    (with-temp-buffer
      (insert output)
      (message "Buffer content after insert: %s"
               (buffer-string))
      (goto-char
       (point-min))
      (while
          (re-search-forward "F821 undefined name '\\([^']+\\)'" nil t)
        (let
            ((name
              (match-string 1)))
          (message "Found undefined name: %s" name)
          (when
              (assoc name pim-import-list)
...
(defun pim--find-import-position
    (name current-offset)
  "Find proper position to insert import for NAME, considering CURRENT-OFFSET."
  (message "Finding import position for %s (offset: %d)" name
           current-offset)
  (save-excursion
    (let*
        ((main-guard-pos
        ...
        (let*
            ((import-line
             ...
             (pos
              (progn
                (message
                 "Finding import position for %s with offset %d" name
                 current-offset)
                (pim--find-import-position name current-offset)))
             (main-guard-pos
              (save-excursion
                (goto-char pos)
                (re-search-backward "^if __name__ == \"__main__\":"
                                    nil t))))
                ...
                ((indented-line
                  (if
                      ...
                      (progn
                        ...
                        (concat "    " import-line))
                    import-line)))
              (message "Adding line to position %d: %s" pos
                       indented-line)
              (push indented-line
                    (gethash pos import-positions))
              (setq current-offset
                    (+ current-offset
                       (1+
                        (length indented-line))))))))
...
(defun pim--find-multiple-imports-one-line
  ...
  (let
      ((temp-buffer
        (generate-new-buffer "*pim-single-line-imports*")))
    (save-excursion
      (goto-char
       (point-min))
      (while
...
(defun pim--find-multiple-imports-to-temp-buffer
  ...
  (let
      ((temp-buffer
        (generate-new-buffer "*pim-multiple-imports*")))
    (save-excursion
      (goto-char
       (point-min))
      (while
...
(defun pim--extract-module-and-imports
    (import-statement)
  ...
  (when
    ...
    (let
        ((module
          (match-string 1 import-statement))
         (imports
          (match-string 2 import-statement)))
      (cons module
            (split-string imports ",[\n \t]*" t "[ \t\n]+")))))

(defun pim--format-imports
    (module-imports)
...
(defun pim--find-imports-marker
  ...
  (save-excursion
    (goto-char
     (point-min))
    (if
        (re-search-forward "^\"\"\"Imports\"\"\"$" nil t)
        (progn
          (forward-line 1)
          (point))
      (pim--find-header-end))))

(defun pim--insert-imports-at-top
    (imports)
  "Insert IMPORTS after the Imports marker or at header end."
  (save-excursion
    (goto-char
     (pim--find-imports-marker))
    (insert
     (string-join imports "\n")
     "\n\n")))
;;;###autoload
...
(when
    (not load-file-name)
  (message "python-import-manager.el loaded."
